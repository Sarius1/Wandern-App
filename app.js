...
function renderSavedAreas(trip, container) {
  ...
  container.innerHTML = areas.map(a => `
    <div class="offline-area-row" data-area-id="${esc(a.id)}">
  ...
  container.querySelectorAll('[data-del-area]').forEach(btn => {
    btn.onclick = async () => {
      ...
      const rows = container.querySelectorAll('.offline-area-row');
      if (rows.length === 0) container.innerHTML = `<p class="offline-empty">No areas saved yet.</p>`;
