...
function renderMapTab(root, trip) {
  ...
  root.querySelector('#btn-offline').onclick = () => openOfflineModal(root);
...
function initMap(trip) {
  if (_map) { _map.remove(); _map = null; _routeLayer = null; _locationMarker = null; }
...
function openOfflineModal(mapRoot) {
  if (!_map) return;
  ...
  const estimate = countTiles(minZ, maxZ);

  openModal(`
    <div class="modal-title">Save map for offline use</div>
    <div class="field">
      <label>This saves the currently visible area.<br>Zoom in on your hiking region first for best results.</label>
    </div>
    <div class="field">
      ...
      <div id="m-tile-count" style="font-size:.8rem;color:var(--text-muted);margin-top:6px;">~${estimate} tiles to download</div>
    </div>
    ...
    <div class="modal-actions">
      <button class="btn-secondary" id="m-dl-cancel" onclick="closeModal()">Cancel</button>
  ...
  const updateCount = () => {
    ...
    if (mx < mn) return;
    const n = countTiles(mn, mx);
    document.getElementById('m-tile-count').textContent = `~${n} tiles to download`;
  };
  ...
  document.getElementById('m-dl-start').onclick = async () => {
    ...
    if (urls.length > 8000) {
      if (!confirm(`${urls.length} tiles is a large download (~${Math.round(urls.length*25/1024)}MB). Continue?`)) return;
    }

    document.getElementById('m-dl-start').disabled = true;
    document.getElementById('m-dl-cancel').textContent = 'Close';
    ...
    status.textContent = failed > 0
      ? `Done — ${done - failed} saved, ${failed} failed`
      : `All ${done} tiles saved for offline use`;
